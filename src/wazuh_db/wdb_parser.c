    ...
    if (!agent_data) {
        ...
        if (cJSON_IsNumber(j_id)) {
            ...
            if (OS_SUCCESS != wdb_global_update_agent_version(wdb, id, os_name, os_version, os_major, os_minor, os_codename,
                ...
                wdbc_result result = wdb_parse_global_set_agent_labels(wdb, labels_data, output);
...
wdbc_result wdb_parse_global_get_agent_labels(wdb_t* wdb, char* input, char** output) {
    int agent_id = 0;
    cJSON *labels = NULL;
    ...
    *output = cJSON_PrintUnformatted(labels);
    cJSON_Delete(labels);
