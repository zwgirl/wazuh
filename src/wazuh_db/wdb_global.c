...
cJSON* wdb_global_get_agents_to_disconnect(wdb_t *wdb, int last_agent_id, int keep_alive, wdbc_result* status) {
    sqlite3_stmt* stmt = NULL;
    ...
    cJSON_ArrayForEach(agent, result) {
        ...
        if (cJSON_IsNumber(id)) {
            ...
            if (OS_SUCCESS != wdb_global_update_agent_connection_status(wdb, id->valueint, "disconnected")) {
