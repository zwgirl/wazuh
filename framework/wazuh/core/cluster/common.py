...
class Handler(asyncio.Protocol):
    ...
    async def send_file(self, filename: str) -> bytes:
        ...
        """
        #TODO-CLUSTER This function should not be calculating pahts, just sending the file
        if not os.path.exists(filename):
            raise exception.WazuhClusterError(3034, extra_message=filename)
        #TODO-CLUSTER Why remove /var/ossec after binary encoding???
        filename = filename.encode()
        relative_path = filename.replace(common.ossec_path.encode(), b'')
        #TODO-CLUSTER Why save all the responses?
