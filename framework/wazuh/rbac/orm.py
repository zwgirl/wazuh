...
class AuthenticationManager:
    ...
    def update_user(self, user_id: int, password: str, allow_run_as: bool, resource_type: ResourceType = None) -> bool:
...
class RolesPoliciesManager:
    ...
    def add_policy_to_role(self, role_id: int, policy_id: int, level: int = None, created_at: DateTime = None, position: int = None,
        ...
        try:
            ...
            if int(role_id) > max_id_reserved or force_admin:
                ...
                if position is not None or self.session.query(
                    ...
                    if level:
                        position = level
                    elif position is None or position > check_max_level(role_id) + 1:
...
class RolesRulesManager:
    ...
    def add_rule_to_role(self, rule_id: int, role_id: int, level: int = None, created_at: DateTime = None, atomic: bool = True,
...
class DatabaseManager:
    ...
    def delete_orphans(self, session):
        query = self.sessions[session].query(UserRoles).filter(or_(UserRoles.user_id.is_(None), UserRoles.role_id.is_(None))).all()
            ...
            with AuthenticationManager(self.sessions[database]) as auth:
                for d_username, payload in default_users[next(iter(default_users))].items():
                    auth.add_user(username=d_username, password=payload['password'],
                                  allow_run_as=payload['allow_run_as'], resource_type=resource_type, check_default=False)
        ...
        with open(os.path.join(default_path, 'rules.yaml'), 'r') as stream:
            ...
            with RulesManager(self.sessions[database]) as rum:
                for d_rule_name, payload in default_rules[next(iter(default_rules))].items():
                    rum.add_rule(name=d_rule_name, rule=payload['rule'], resource_type=resource_type, check_default=False)
            ...
            with PoliciesManager(self.sessions[database]) as pm:
                for d_policy_name, payload in default_policies[next(iter(default_policies))].items():
                    for name, policy in payload['policies'].items():
                        pm.add_policy(name=f'{d_policy_name}_{name}', policy=policy, resource_type=resource_type, check_default=False)
    ...
    def migrate_data(self, source, target, from_id: int = None, to_id: int = None, resource_type: ResourceType = None):
        ...
        with AuthenticationManager(self.sessions[target]) as auth_manager:
            for user in old_users:
                auth_manager.add_user(username=user[0], password=user[1], allow_run_as=user[2], created_at=datetime.strptime(user[3], '%Y-%m-%d %H:%M:%S.%f'),
                                      resource_type=resource_type, check_default=True)
        logger.error('--> roles')
        ...
        with RolesManager(self.sessions[target]) as role_manager:
            for role in old_roles:
                role_manager.add_role(name=role[0], created_at=datetime.strptime(role[1], '%Y-%m-%d %H:%M:%S.%f'), resource_type=resource_type, check_default=True)
        ...
        with RulesManager(self.sessions[target]) as rule_manager:
            for rule in old_rules:
                rule_manager.add_rule(name=rule[0], rule=rule[1], created_at=datetime.strptime(rule[2], '%Y-%m-%d %H:%M:%S.%f'), check_default=True)
        logger.error('--> policies')
        ...
        with PoliciesManager(self.sessions[target]) as policy_manager:
            for policy in old_policies:
                policy_manager.add_policy(name=policy[0], policy=policy[1], created_at=datetime.strptime(policy[2], '%Y-%m-%d %H:%M:%S.%f'),
                                          check_default=True)
        logger.error('--> reelationships')
        ...
        with UserRolesManager(self.sessions[target]) as user_role_manager:
            for user_role in old_user_roles:
                user_role_manager.add_role_to_user(user_id=user_role[0],  role_id=user_role[1], level=user_role[2],
                                                   created_at=datetime.strptime(user_role[3], '%Y-%m-%d %H:%M:%S.%f'),
                                                   force_admin=True)
        ...
        with RolesPoliciesManager(self.sessions[target]) as role_policy_manager:
            for role_policy in old_roles_policies:
                role_policy_manager.add_policy_to_role(role_id=role_policy[0],
                                                       ...
                                                       level=role_policy[2],
                                                       created_at=datetime.strptime(role_policy[3], '%Y-%m-%d %H:%M:%S.%f'),
                                                       force_admin=True)
        ...
        with RolesRulesManager(self.sessions[target]) as role_rule_manager:
            for role_rule in old_roles_rules:
                role_rule_manager.add_rule_to_role(role_id=role_rule[0],
                                                   rule_id=role_rule[1],
                                                   created_at=datetime.strptime(role_rule[2], '%Y-%m-%d %H:%M:%S.%f'),
                                                   force_admin=True)
        logger.error('--> done')
...
try:
    ...
    # If database does not exists it means this is a fresh installation and must be created properly
        ...
        logger = logging.getLogger('wazuh')
        logger.error("---TEsting No existe")
        db_manager.connect(_auth_db_file)
    ...
    logger.error('Error during the database migration. Restoring the database.')
